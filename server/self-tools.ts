...
export function createSelfMcp() {
  return createSdkMcpServer({
    ...
    tools: [
      tool(
        "get_config",
        "Return Kizuna Agent's runtime configuration: which Codex model it's using, which integrations are loaded, and basic env info. Use when the user asks 'what model are you?', 'what version?', or anything about the agent itself.",
        {},
        async () => {
      ...
      tool(
        "set_model",
        `Switch the Codex model used by Kizuna. The change applies to the *next* turn (this turn finishes on the current model). Accepts either a canonical ID or a friendly alias.

Aliases: ${Object.keys(MODEL_ALIASES).map((k) => `"${k}"`).join(", ")}
Canonical: ${[...KNOWN_MODELS].map((k) => `"${k}"`).join(", ")}

Use when the user says "use codex/default", "use gpt-5", or "make it faster".`,
        {
          model: z
            .string()
            .describe('Model to use. Canonical ID like "gpt-5-codex" or alias like "codex".'),
        },
        async ({ model }) => {
      ...
      tool(
        "list_integrations",
        "List the user's currently connected integrations (Gmail, Notion, etc.) with the actual account behind each connection. Use when the user asks 'what tools do I have connected?' or 'which Gmail account?' or 'what integrations are set up?'.",
        {},
        async () => {
      ...
      tool(
        "inspect_toolkit",
        "Look up a specific Composio toolkit by exact slug. Returns whether it exists, whether it's currently connected, and (if requested) the list of tools it exposes. Use when the user asks 'what can the Gmail tool do?' or 'is Notion connected?'.",
        {
          slug: z
            .string()
            .describe("Exact toolkit slug, e.g. 'gmail', 'notion', 'linear'. Lowercase."),
          includeTools: z
            .boolean()
            .optional()
            .default(false)
