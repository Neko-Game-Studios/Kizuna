...
Notable changes per release. `[BREAKING]` entries require action on your fork — `/upgrade-kizuna` will surface these and offer to run the relevant migration skill.
...
- Added: `server/self-tools.ts` — interaction-agent MCP server (`kizuna-self`) exposing `get_config`, `list_integrations`, `search_composio_catalog`, `inspect_toolkit`, and `set_model`. Lets the user ask Kizuna about its own configuration from iMessage without spawning a sub-agent.
- Added: runtime model override via `convex/settings.ts` + `server/runtime-config.ts`. Both the dispatcher and execution agent now read the model from a Convex-backed setting on each turn (with 30s in-memory cache and `KIZUNA_MODEL` fallback). User can say "use opus" / "switch to sonnet" / "make it faster (haiku)" from iMessage; takes effect on the next turn. Aliases: `opus`, `sonnet`, `haiku`.
...
- Added: env vars `COMPOSIO_API_KEY`, `COMPOSIO_USER_ID` (optional, defaults to `kizuna-default`).
- Added: `/upgrade-kizuna` Claude Code skill for bringing upstream changes into a customized fork.
