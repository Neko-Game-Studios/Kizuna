...
export function DashboardPanel({ isDark }: { isDark: boolean }) {
  ...
  const c = {
    ...
    value: "text-[var(--ink)]",
    sub: "text-[var(--muted)]",
    chart: "editorial-card-soft",
  };

  const failPct = (filtered.agents.failureRate * 100).toFixed(1);

  return (
    <div className="editorial-panel-shell h-full overflow-y-auto debug-scroll p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="editorial-label">
          Overview
        </h2>
        <div className="flex items-center rounded-full border border-[var(--hairline)] bg-[var(--canvas)] text-xs p-0.5">
          {RANGES.map((r) => (
            <button
              key={r.id}
              onClick={() => setRange(r.id)}
              className={`px-3 py-1.5 transition-colors rounded-full ${
                range === r.id
...
function StatCard({
  ...
  return (
    <div className={`p-4 relative ${c.card}`}>
        ...
        {info && (
          <div className="relative" ref={popRef}>
            <button
              ...
              aria-expanded={open}
              className="w-3.5 h-3.5 inline-flex items-center justify-center rounded-full border border-[var(--hairline)] text-[9px] font-bold normal-case tracking-normal transition-colors text-[var(--muted)] hover:border-[var(--link)] hover:text-[var(--link)]"
            >
              i
            </button>
            {open && (
              <div
                ...
                className="absolute z-30 left-0 top-full mt-1.5 w-64 editorial-card px-3 py-2.5 text-[11px] leading-snug normal-case tracking-normal bg-[var(--canvas)]"
              >
                <div className="font-semibold mb-1 text-[var(--ink)]">
                  {info.title}
                </div>
      ...
      <div className={`text-xl font-semibold mono mt-1 ${color ?? c.value}`}>
        {value}
      </div>
