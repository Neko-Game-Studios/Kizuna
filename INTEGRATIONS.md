...
Kizuna's integrations are provided by [Composio](https://composio.dev/?utm_source=chris&utm_medium=youtube&utm_campaign=collab), a tool-aggregator that exposes 1000+ third-party services (Gmail, GitHub, Slack, Notion, Linear, Google Drive, HubSpot, Salesforce, …) behind one API.
...
## How it hooks into Kizuna

Each connected Composio toolkit is registered in Kizuna's integration registry (`server/integrations/registry.ts`) keyed by its slug. When the dispatcher calls:
...
When you click Connect on one of those, Kizuna surfaces an amber banner explaining that you need to:
...
This is a one-time setup per toolkit (not per user) — all users of your Kizuna instance reuse the same auth config after that.

## Notes

- **Single-tenant by default.** All connections are keyed under `COMPOSIO_USER_ID` (defaults to `kizuna-default`). Override if you manage Composio sessions elsewhere and want Kizuna to share that user.
...
- **No tokens live in Kizuna.** Composio stores OAuth credentials on their side. Kizuna never sees them.
